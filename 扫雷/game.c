...
{
	int i, j;
	char col[19] = "１２３４５６７８９"; //全角数字
	char row[41] = "ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴ"; //全角字母
	setcl(MAP_COLOR);
	for (i = 0; i < size; i++) //打印左右边界
	{
		setrd(2, i + 2);//左边界
		if (i < 9) // i+1<10 打印全角数字
...
void mine_clearance(int x, int y, short map[COL_MAX][COL_MAX], short display[COL_MAX][COL_MAX], int size)
{
	...
	if (x != size - 1 && display[x + 1][y] == 0) //向下搜索
	{
		if (map[x + 1][y] == 0)
			mine_clearance(x + 1, y, map, display, size);
		else if (map[x + 1][y] < 9)
	...
	if (y != size - 1 && display[x][y + 1] == 0) //向右搜索
	{
		if (map[x][y + 1] == 0)
			mine_clearance(x, y + 1, map, display, size);
		else if (map[x][y + 1] < 9)
